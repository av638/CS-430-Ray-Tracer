...
static inline double freselCalc(double a, double b, double mix)
{
...
void trace(Object objects[], double* Ro, double* Rd, int numObjects, int currDepth, int currObject, double* createdColor)
{
    //double* createdColor = malloc(sizeof(double)*3);
    ...
    {
        double t = 0;
        // If we intersect with the current object that we are trying to find the reflection/refraction
        // then skip it
        if(currObject == i) continue;
        // otherwise find the intersections with other objects
        else
        {
        ...
        if(best_t > 0 && best_t != INFINITY && best_t != -1)
        {
            ...
            {  // printf("hello there!");
                ...
                double refract = objects[best_i].properties.sphere.refractivity;
                ...
                if((reflect >= 0 || refract >= 0 || indexOfRefraction >=0) && currDepth < 7)
                {
                    ...
                    trace(objects,nextRayO, refldir, numObjects, currDepth+1, best_i, reflection);
        ...
        { // printf("no intersection!!\n");
            ...
            createdColor[0] = 0*255 ;
            createdColor[1] = 0*255;
            createdColor[2] = 0*255;
